...
func ProcStdData(excelData [][]string, accessToken string) {
	for _, ed := range excelData {
		su := models.StdUser{}
		if len(ed) == 3 {
			status, stErr := strconv.Atoi(ed[2])
			if stErr != nil {
				logs.Error("ProcStdData, stErr: ", stErr)
				status = 1
			}
			su.GitUserId = ed[0]
			switch status {
			case 3:
				delErr := models.DelStdUser(&su, "GitUserId")
				if delErr != nil {
					logs.Error("DelStdUser, delErr: ", delErr)
				}
			default:
				gui := GetGitId(accessToken, ed[0])
				if gui.GitId == 0 {
...
func ProcTutData(excelData [][]string, accessToken string) {
	for _, ed := range excelData {
		...
		if len(ed) == 3 {
			status, stErr := strconv.Atoi(ed[2])
			if stErr != nil {
				logs.Error("ProcTutData, stErr: ", stErr)
				status = 1
			}
			tu.GitUserId = ed[0]
			switch status {
			case 3:
				delErr := models.DelTutUser(&tu, "GitUserId")
				if delErr != nil {
